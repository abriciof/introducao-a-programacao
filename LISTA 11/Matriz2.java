...
public class Matriz
{
    ...
    public void somaValores()
    {
        ...
        for(int contLinha = 0; contLinha < this.getLinhas(); contLinha++)
        {
            ...
            {
                somaTotal = somaTotal + this.getElemento(contLinha, contColuna);
                if(contLinha == contColuna)
                {
                    somaDiagonal = somaDiagonal + this.getElemento(contLinha, contColuna);
                }
            }
        }
        System.out.printf("Soma Matriz: %d\n", somaTotal);
        System.out.printf("Soma Diagonal: %d", somaDiagonal);
    }
